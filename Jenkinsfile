pipeline {
    ...
    stages {
        ...
        stage('4. Generate PDFs') {
            steps {
                sh """
                    docker run --rm -v \$(pwd):/workspace -w /workspace python:3.12-slim sh -c "pip install --no-cache-dir markdown 'svglib==1.5.1' xhtml2pdf requests && python3 scripts/export_pdf.py"
                """
            }
        }
